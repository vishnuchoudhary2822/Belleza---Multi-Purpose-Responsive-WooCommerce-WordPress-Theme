 ...
 */ 
    ...
    function fashion_setup()
    {
        ...
        register_nav_menus(array(
            'menu-1' => esc_html__('Primary', 'fashion')
        ));

        register_nav_menus(array(
            'top-nav' => esc_html__('Top Nav', 'fashion')
        ));
 ...
 */
function fashion_content_width()
{
    $GLOBALS['content_width'] = apply_filters('fashion_content_width', 640);
}
add_action('after_setup_theme', 'fashion_content_width', 0);
...
function fashion_widgets_init()
{
    register_sidebar(array(
        'name' => esc_html__('Sidebar', 'fashion'),
        'id' => 'sidebar-1',
        'description' => esc_html__('Add widgets here.', 'fashion'),
    ...
    register_sidebar(array(
        'name' => esc_html__('Logo & Address', 'fashion'),
        'id' => 'footer-1',
        'description' => esc_html__('Appears in the footer area', 'fashion'),
        'before_widget' => '',
        'after_widget' => '',
    ...
    register_sidebar(array(
        'name' => esc_html__('Social Icons & Email', 'fashion'),
        'id' => 'footer-2',
        'description' => esc_html__('Appears in the footer area', 'fashion'),
        'before_widget' => '',
        'after_widget' => '',
    ...
    register_sidebar(array(
        'name' => esc_html__('Top-Bar-Left', 'fashion'),
        'id' => 'top-bar',
        'description' => esc_html__('Appears in the Top-Bar', 'fashion'),
        'before_widget' => '',
        'after_widget' => '',
        'before_title' => '<h3 class="widget-title">',
        'after_title' => '</h3>'
    ...
    register_sidebar(array(
        'name' => esc_html__('Shop Sidebar', 'fashion'),
        'id' => 'shop-sidebar',
        'description' => esc_html__('Add widgets here.', 'fashion'),
        'before_widget' => '<section id="%1$s" class="widget %2$s">',
        'after_widget' => '</section>',
    ...
    register_sidebar(array(
        'name' => esc_html__('Theme Feature', 'fashion'),
        'id' => 'theme-feature',
        'description' => esc_html__('Add widgets here.', 'fashion'),
        'before_widget' => '<section id="%1$s" class="widget %2$s">',
        'after_widget' => '</section>',
...
function fashion_google_fonts_url() {
    $font_url = '';

    if ( 'off' !== _x( 'on', 'Google font: on or off', 'fashion' ) ) {
...
function fashion_scripts()
{
    ...
    /* Theme CSS */
    wp_enqueue_style('fashion-default-theme-style', get_template_directory_uri() . '/css/default-theme-style.css');
    wp_enqueue_style('google-fonts-family', fashion_google_fonts_url(), array(), '1.0.0' );
	wp_enqueue_style('flaticon', get_template_directory_uri() . '/css/flaticon.css');
    wp_enqueue_style('fashion-style', get_template_directory_uri() . '/css/style.css');
    ...
    if(get_the_ID() == 1825){
    wp_enqueue_style('fashion-custom-base', get_template_directory_uri() . '/css/custom-base.css');
    }
    /* Default CSS */
    ...
    //fashion css
    wp_enqueue_style( 'fashion-default-at', get_template_directory_uri() . '/css/default-at-style.css');
    //wp_enqueue_style('fashion-stylesheet', 'http://multipurposethemes.com/wordpressdemo/fashion/wp-content/themes/fashion/style.css');
    wp_enqueue_style('fashion-stylesheet', get_template_directory_uri() . '/css/fashion-style.css');


    // Add Multipurpose js
    //wp_enqueue_script( 'fashion-customjs', get_stylesheet_directory_uri() . '/js/multipurpose.js');
    ...
    //navigation js
    wp_enqueue_script( 'fashion-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '1.0', true );    
    ...
    wp_enqueue_script('fashion-scrollreveal', get_template_directory_uri() . '/vendor/scrollreveal/scrollreveal.min.js', array(), '', true);
    
    /* Theme JavaScript */
    wp_enqueue_script('fashion-theme', get_template_directory_uri() . '/js/theme.js', array(), '', true);
...
function fashion_scripts_footer()
{
    wp_enqueue_script('fashion-js');
}
add_action('wp_footer', 'fashion_scripts_footer');


/* Theme Options 

function fashion_add_theme_menu_item()
{
    add_theme_page("Theme Options", "Theme Options", "manage_options", "theme-panel", "fashion_theme_settings_page", null, 99);
}

add_action("admin_menu", "fashion_add_theme_menu_item");

function fashion_theme_settings_page()
{
?>
       <div class="wrap">
        <h1>Theme Panel</h1>
...
function fashion_handle_logo_upload()
{
...
function fashion_change_font()
{
    $load_fonts = fashion_google_fonts();
...
function fashion_theme_panel_fields()
{
    add_settings_section("section", "All Settings", null, "theme-options");
    
    add_settings_field("change_font", "Google Font Family", "fashion_change_font", "theme-options", "section");
    add_settings_field("logo", "Top Menu Logo", "fashion_logo_display", "theme-options", "section");
    
    register_setting("section", "change_font");
    register_setting("section", "logo", "fashion_handle_logo_upload");
}

add_action("admin_init", "fashion_theme_panel_fields");
...
if (!function_exists('fashion_get_google_fonts')):
    function fashion_get_google_fonts()
    {
...
add_action('wp_enqueue_scripts', 'fashion_get_google_fonts');

function fashion_theme_options(){
    $custom_css        = "";
    $theme_font_family = (get_option('change_font') != '') ? get_option('change_font') : '';
    ...
    /* Custom CSS */
	//wp_enqueue_style('fashion-custom-style', get_template_directory_uri() . '/css/custom.css');
    wp_add_inline_style('fashion-custom-style', $custom_css);
    
}
add_action('wp_enqueue_scripts', 'fashion_theme_options');
    ...
    if ( ( isset( $ajax ) && $ajax ) || ( property_exists( $args, 'theme_location' ) && $args->theme_location === 'menu-1' ) ) {
        ...
        if ( class_exists( 'woocommerce' ) ) {
            ...
            $items .= '<li class="' . esc_attr( $css_class ) . '">';
                $items .= '<a class="cart-contents" href="' . esc_url(wc_get_cart_url() ) . '"><i class="fa fa-shopping-bag" aria-hidden="true"></i> ';
                    $items .= wp_kses_data( WC()->cart->get_cart_total() ) . ' - <span class="count">' .  wp_kses_data( sprintf( _n( '%d item', '%d items', WC()->cart->get_cart_contents_count(), 'fashion' ), WC()->cart->get_cart_contents_count() ) ) . '</span>';
                $items .= '</a>';
            $items .= '</li>';
        }
        // Easy Digital Downloads
            ...
            $items .= '<li class="' . esc_attr( $css_class ) . '">';
                $items .= '<a class="cart-contents" href="' . esc_url( edd_get_checkout_uri() ) . '"><i class="fa fa-shopping-bag" aria-hidden="true"></i> ';
                    $items .= wp_kses_data( edd_cart_subtotal() ) . ' - <span class="count">' .  wp_kses_data( sprintf( _n( '%d item', '%d items', edd_get_cart_quantity(), 'fashion' ), edd_get_cart_quantity() ) ) . '</span>';
                $items .= '</a>';
            $items .= '</li>';
        }
    }
...
function fashion_custom_excerpt_more($more) {
      global $post;
      if ($post->post_type == 'post'){
   ...
   add_filter('excerpt_more', 'fashion_custom_excerpt_more');
/*animated number counter script*/



/* +========================ot_theme_mode  Added By Dev8 on 15/09/2017===========================================*/
function fashion_theme_file() {

	require_once( get_template_directory() . '/inc/fashion-theme-option.php' );

}
add_action('wp_head', 'fashion_theme_file');
/**
...
function ocdi_import_files() {
  return array(
    array(
      ...
      'import_file_url'            => 'http://html-templates.multipurposethemes.com/wordpress/themes/data/fashion/demo-content.xml',
      'import_widget_file_url'     => 'http://html-templates.multipurposethemes.com/wordpress/themes/data/fashion/widgets.wie',
      'import_customizer_file_url' => 'http://html-templates.multipurposethemes.com/wordpress/themes/data/fashion/customizer.dat',
      'import_preview_image_url'   => 'http://html-templates.multipurposethemes.com/wordpress/themes/data/fashion/screenshot.png',
      'import_notice'              => __( 'After you import this demo, you will have to setup the slider separately.', 'fashion' ),
      'preview_url'                => 'https://www.multipurposethemes.com/wordpressdemo/fashion/fashion/',
    ),
  );
}
