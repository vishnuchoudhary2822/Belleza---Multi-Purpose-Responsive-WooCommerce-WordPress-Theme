...
function jewelry_entry_footer() {
	...
	if ( 'post' === get_post_type() ) {
		...
		if ( $categories_list && jewelry_categorized_blog() ) {
			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'jewelry' ) . '</span>', $categories_list ); // WPCS: XSS OK.
		}
	...
	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( sprintf( wp_kses( __( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'jewelry' ), array( 'span' => array( 'class' => array() ) ) ), get_the_title() ) );
		echo '</span>';
	}
