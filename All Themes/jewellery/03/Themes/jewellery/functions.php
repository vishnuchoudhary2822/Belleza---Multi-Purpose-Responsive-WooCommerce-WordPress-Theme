 ...
 */ 
    ...
    function jewelry_setup()
    {
        ...
        register_nav_menus(array(
            'top-nav' => esc_html__('Top Nav', 'jewelry')
        ));
...
function jewelry_widgets_init()
{
    ...
    register_sidebar(array(
        'name' => esc_html__('Logo & Address', 'jewelry'),
        'id' => 'footer-1',
        'description' => esc_html__('Appears in the footer area', 'jewelry'),
        'before_widget' => '',
        'after_widget' => '',
    ...
    register_sidebar(array(
        'name' => esc_html__('Social Icons & Email', 'jewelry'),
        'id' => 'footer-2',
        'description' => esc_html__('Appears in the footer area', 'jewelry'),
        'before_widget' => '',
        'after_widget' => '',
    ...
    register_sidebar(array(
        'name' => esc_html__('Top-Bar-Left', 'jewelry'),
        'id' => 'top-bar',
        'description' => esc_html__('Appears in the Top-Bar', 'jewelry'),
        'before_widget' => '',
        'after_widget' => '',
        'before_title' => '<h3 class="widget-title">',
        'after_title' => '</h3>'
    ...
    register_sidebar(array(
        'name' => esc_html__('Footer 1', 'jewelry'),
        'id' => 'shop-footer-1',
        'description' => esc_html__('Appears in the footer area', 'jewelry'),
        'before_widget' => '',
        'after_widget' => '',
    ...
    register_sidebar(array(
        'name' => esc_html__('Footer 2', 'jewelry'),
        'id' => 'shop-footer-2',
        'description' => esc_html__('Appears in the footer area', 'jewelry'),
        'before_widget' => '',
        'after_widget' => '',
    ...
    register_sidebar(array(
        'name' => esc_html__('Footer 3', 'jewelry'),
        'id' => 'shop-footer-3',
        'description' => esc_html__('Appears in the footer area', 'jewelry'),
        'before_widget' => '',
        'after_widget' => '',
    ...
    register_sidebar(array(
        'name' => esc_html__('Footer 4', 'jewelry'),
        'id' => 'shop-footer-4',
        'description' => esc_html__('Appears in the footer area', 'jewelry'),
        'before_widget' => '',
        'after_widget' => '',
    ...
    register_sidebar(array(
        'name' => esc_html__('Shop Sidebar', 'jewelry'),
        'id' => 'shop-sidebar',
        'description' => esc_html__('Add widgets here.', 'jewelry'),
        'before_widget' => '<section id="%1$s" class="widget %2$s">',
        'after_widget' => '</section>',
    ...
    register_sidebar(array(
        'name' => esc_html__('Theme Feature', 'jewelry'),
        'id' => 'theme-feature',
        'description' => esc_html__('Add widgets here.', 'jewelry'),
        'before_widget' => '<section id="%1$s" class="widget %2$s">',
        'after_widget' => '</section>',
...
function jewelry_google_fonts_url() {
    $font_url = '';

    if ( 'off' !== _x( 'on', 'Google font: on or off', 'jewelry' ) ) {
...
function jewelry_scripts()
{
    ...
    /* Theme CSS */
    wp_enqueue_style('jewelry-default-theme-style', get_template_directory_uri() . '/css/default-theme-style.css');
    wp_enqueue_style('google-fonts-family', jewelry_google_fonts_url(), array(), '1.0.0' );
	wp_enqueue_style('flaticon', get_template_directory_uri() . '/css/flaticon.css');
    wp_enqueue_style('jewelry-style', get_template_directory_uri() . '/css/style.css');
    wp_enqueue_style('jewelry-custom', get_template_directory_uri() . '/css/custom.css');
    ...
    if(get_the_ID() == 1825){
    wp_enqueue_style('jewelry-custom-base', get_template_directory_uri() . '/css/custom-base.css');
    }
    /* Default CSS */
    ...
    //jewelry css
    wp_enqueue_style( 'jewelry-default-at', get_template_directory_uri() . '/css/default-at-style.css');
    //wp_enqueue_style('jewelry-stylesheet', 'http://multipurposethemes.com/wordpressdemo/jewelry/wp-content/themes/jewelry/style.css');
    wp_enqueue_style('jewelry-stylesheet', get_template_directory_uri() . '/css/jewelry-style.css');

    //navigation js
   wp_enqueue_script( 'jewelry-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '1.0', true );

    // Add Multipurpose js
   //wp_enqueue_script( 'jewelry-customjs', get_stylesheet_directory_uri() . '/js/multipurpose.js');
    ...
    wp_enqueue_script('jewelry-scrollreveal', get_template_directory_uri() . '/vendor/scrollreveal/scrollreveal.min.js', array(), '', true);
    
    /* Theme JavaScript */
    wp_enqueue_script('jewelry-theme', get_template_directory_uri() . '/js/theme.js', array(), '', true);
...
function jewelry_scripts_footer()
{
    wp_enqueue_script('jewelry-js');
}
add_action('wp_footer', 'jewelry_scripts_footer');


/* Theme Options 

function jewelry_add_theme_menu_item()
{
    add_theme_page("Theme Options", "Theme Options", "manage_options", "theme-panel", "jewelry_theme_settings_page", null, 99);
}

add_action("admin_menu", "jewelry_add_theme_menu_item");

function jewelry_theme_settings_page()
{
?>
       <div class="wrap">
        <h1>Theme Panel</h1>
...
function jewelry_handle_logo_upload()
{
...
function jewelry_change_font()
{
    $load_fonts = jewelry_google_fonts();
...
function jewelry_theme_panel_fields()
{
    add_settings_section("section", "All Settings", null, "theme-options");
    
    add_settings_field("change_font", "Google Font Family", "jewelry_change_font", "theme-options", "section");
    add_settings_field("logo", "Top Menu Logo", "jewelry_logo_display", "theme-options", "section");
    
    register_setting("section", "change_font");
    register_setting("section", "logo", "jewelry_handle_logo_upload");
}

add_action("admin_init", "jewelry_theme_panel_fields");
...
if (!function_exists('jewelry_get_google_fonts')):
    function jewelry_get_google_fonts()
    {
...
add_action('wp_enqueue_scripts', 'jewelry_get_google_fonts');

function jewelry_theme_options(){
    $custom_css        = "";
    $theme_font_family = (get_option('change_font') != '') ? get_option('change_font') : '';
    ...
    /* Custom CSS */
	//wp_enqueue_style('jewelry-custom-style', get_template_directory_uri() . '/css/custom.css');
    wp_add_inline_style('jewelry-custom-style', $custom_css);
    
}
add_action('wp_enqueue_scripts', 'jewelry_theme_options');
        ...
        if ( class_exists( 'woocommerce' ) ) {
            ...
            $items .= '<li class="' . esc_attr( $css_class ) . '">';
                $items .= '<a class="cart-contents" href="' . esc_url( WC()->cart->get_cart_url() ) . '"><i class="fa fa-shopping-bag" aria-hidden="true"></i> ';
                    $items .= wp_kses_data( WC()->cart->get_cart_total() ) . ' - <span class="count">' .  wp_kses_data( sprintf( _n( '%d item', '%d items', WC()->cart->get_cart_contents_count(), 'jewelry' ), WC()->cart->get_cart_contents_count() ) ) . '</span>';
                $items .= '</a>';
            $items .= '</li>';
        }
        // Easy Digital Downloads
            ...
            $items .= '<li class="' . esc_attr( $css_class ) . '">';
                $items .= '<a class="cart-contents" href="' . esc_url( edd_get_checkout_uri() ) . '"><i class="fa fa-shopping-bag" aria-hidden="true"></i> ';
                    $items .= wp_kses_data( edd_cart_subtotal() ) . ' - <span class="count">' .  wp_kses_data( sprintf( _n( '%d item', '%d items', edd_get_cart_quantity(), 'jewelry' ), edd_get_cart_quantity() ) ) . '</span>';
                $items .= '</a>';
            $items .= '</li>';
        }
    }
...
function jewelry_custom_excerpt_more($more) {
      global $post;
      if ($post->post_type == 'post'){
   ...
   add_filter('excerpt_more', 'jewelry_custom_excerpt_more');
/*animated number counter script*/



/* +========================ot_theme_mode  Added By Dev8 on 15/09/2017===========================================*/
function jewelry_theme_file() {

	require_once( get_template_directory() . '/inc/jewelry-theme-option.php' );

}
add_action('wp_head', 'jewelry_theme_file');
/**
...
function ocdi_import_files() {
  return array(
    array(
      ...
      'import_file_url'            => 'http://html-templates.multipurposethemes.com/wordpress/themes/data/jewellery/03/demo-content.xml',
      'import_widget_file_url'     => 'http://html-templates.multipurposethemes.com/wordpress/themes/data/jewellery/03/widgets.wie',
      'import_customizer_file_url' => 'http://html-templates.multipurposethemes.com/wordpress/themes/data/jewellery/03/customizer.dat',
      'import_preview_image_url'   => 'http://html-templates.multipurposethemes.com/wordpress/themes/data/jewellery/03/screenshot.png',
      'import_notice'              => __( 'After you import this demo, you will have to setup the slider separately.', 'maximum' ),
      'preview_url'                => 'http://creativewordpressthemes.multipurposethemes.com/jewelry/',
    ),
  );
}
