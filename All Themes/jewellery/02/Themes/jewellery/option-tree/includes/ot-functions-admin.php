...
if ( ! function_exists( 'ot_get_media_post_ID' ) ) {
  ...
  function ot_get_media_post_ID() {
    ...
    if ( $post_ID === false || empty( $post_ID ) ) {
      ...
      $post_ID = $wpdb->get_var($wpdb->prepare("SELECT ID FROM $wpdb->posts WHERE `post_title` = 'Media' AND `post_type` = 'option-tree' AND `post_status` = 'private'", ''));
    ...
    if ( isset( $_POST['import_xml_nonce'] ) && wp_verify_nonce( $_POST['import_xml_nonce'], 'import_xml_form' ) ) {
      ...
      if ( isset( $settings ) && ! empty( $settings ) ) {
        ...
        if ( $file ) {
          ...
          $attachmentid = $wpdb->get_var(  $wpdb->prepare("SELECT ID FROM {$wpdb->posts} WHERE guid='$file'",''));
...
if ( ! function_exists( 'ot_recognized_google_font_families' ) ) {
  ...
  function ot_recognized_google_font_families( $field_id ) {
    ...
    //Comment bypass option tree google fonts. 
      ...
      <div class="option-tree-setting-body">
        ...
        <div class="format-settings">
          ...
          <div class="format-setting type-text wide-desc">
            ...
            <div class="description">' . __( '<strong>Min, Max, & Step</strong>: Add a comma separated list of options in the following format <code>0,100,1</code> (slide from <code>0-100</code> in intervals of <code>1</code>). The three values represent the minimum, fashion, and step options and will only affect the Numeric Slider option type.', 'option-tree' ) . '</div>
...
function ot_fetch_google_fonts( $normalize = true, $force_rebuild = false ) {
  ...
  if ( $force_rebuild || ! is_array( $ot_google_fonts ) || empty( $ot_google_fonts ) ) {
    ...
    $ot_google_fonts_api_key = apply_filters( 'ot_google_fonts_api_key', 'AIzaSyAR3mwM2cyFGkmAA3vAqeGlFNdr9dqAgSU' );
