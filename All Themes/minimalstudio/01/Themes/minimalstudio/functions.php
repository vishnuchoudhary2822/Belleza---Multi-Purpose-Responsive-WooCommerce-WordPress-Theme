 ...
 */ 
    ...
    function minimal_setup()
    {
        ...
        register_nav_menus(array(
            'top-nav' => esc_html__('Top Nav', 'minimal')
        ));
...
function minimal_widgets_init()
{
    ...
    register_sidebar(array(
        'name' => esc_html__('Logo & Address', 'minimal'),
        'id' => 'footer-1',
        'description' => esc_html__('Appears in the footer area', 'minimal'),
        'before_widget' => '',
        'after_widget' => '',
    ...
    register_sidebar(array(
        'name' => esc_html__('Social Icons & Email', 'minimal'),
        'id' => 'footer-2',
        'description' => esc_html__('Appears in the footer area', 'minimal'),
        'before_widget' => '',
        'after_widget' => '',
    ...
    register_sidebar(array(
        'name' => esc_html__('Top-Bar-Left', 'minimal'),
        'id' => 'top-bar',
        'description' => esc_html__('Appears in the Top-Bar', 'minimal'),
        'before_widget' => '',
        'after_widget' => '',
        'before_title' => '<h3 class="widget-title">',
        'after_title' => '</h3>'
    ...
    register_sidebar(array(
        'name' => esc_html__('Shop Sidebar', 'minimal'),
        'id' => 'shop-sidebar',
        'description' => esc_html__('Add widgets here.', 'minimal'),
        'before_widget' => '<section id="%1$s" class="widget %2$s">',
        'after_widget' => '</section>',
    ...
    register_sidebar(array(
        'name' => esc_html__('Theme Feature', 'minimal'),
        'id' => 'theme-feature',
        'description' => esc_html__('Add widgets here.', 'minimal'),
        'before_widget' => '<section id="%1$s" class="widget %2$s">',
        'after_widget' => '</section>',
...
function minimal_google_fonts_url() {
    $font_url = '';

    if ( 'off' !== _x( 'on', 'Google font: on or off', 'minimal' ) ) {
...
function minimal_scripts()
{
    ...
    /* Theme CSS */
    wp_enqueue_style('minimal-default-theme-style', get_template_directory_uri() . '/css/default-theme-style.css');
    wp_enqueue_style('google-fonts-family', minimal_google_fonts_url(), array(), '1.0.0' );
	wp_enqueue_style('flaticon', get_template_directory_uri() . '/css/flaticon.css');
    wp_enqueue_style('minimal-style', get_template_directory_uri() . '/css/style.css');
    wp_enqueue_style('minimal-custom', get_template_directory_uri() . '/css/custom.css');
    ...
    if(get_the_ID() == 1825){
    wp_enqueue_style('minimal-custom-base', get_template_directory_uri() . '/css/custom-base.css');
    }
    /* Default CSS */
    ...
    //minimal css
    wp_enqueue_style( 'minimal-default-at', get_template_directory_uri() . '/css/default-at-style.css');
    //wp_enqueue_style('minimal-stylesheet', 'http://multipurposethemes.com/wordpressdemo/minimal/wp-content/themes/minimal/style.css');
    wp_enqueue_style('minimal-stylesheet', get_template_directory_uri() . '/css/minimal-style.css');

    // Add Multipurpose js
   //wp_enqueue_script( 'minimal-customjs', get_stylesheet_directory_uri() . '/js/multipurpose.js');
    ...
    //navigation js
    wp_enqueue_script( 'minimal-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '1.0', true );    
    ...
    wp_enqueue_script('minimal-scrollreveal', get_template_directory_uri() . '/vendor/scrollreveal/scrollreveal.min.js', array(), '', true);
    
    /* Theme JavaScript */
    wp_enqueue_script('minimal-theme', get_template_directory_uri() . '/js/theme.js', array(), '', true);
...
function minimal_scripts_footer()
{
    wp_enqueue_script('minimal-js');
}
add_action('wp_footer', 'minimal_scripts_footer');


/* Theme Options 

function minimal_add_theme_menu_item()
{
    add_theme_page("Theme Options", "Theme Options", "manage_options", "theme-panel", "minimal_theme_settings_page", null, 99);
}

add_action("admin_menu", "minimal_add_theme_menu_item");

function minimal_theme_settings_page()
{
?>
       <div class="wrap">
        <h1>Theme Panel</h1>
...
function minimal_handle_logo_upload()
{
...
function minimal_change_font()
{
    $load_fonts = minimal_google_fonts();
...
function minimal_theme_panel_fields()
{
    add_settings_section("section", "All Settings", null, "theme-options");
    
    add_settings_field("change_font", "Google Font Family", "minimal_change_font", "theme-options", "section");
    add_settings_field("logo", "Top Menu Logo", "minimal_logo_display", "theme-options", "section");
    
    register_setting("section", "change_font");
    register_setting("section", "logo", "minimal_handle_logo_upload");
}

add_action("admin_init", "minimal_theme_panel_fields");
...
if (!function_exists('minimal_get_google_fonts')):
    function minimal_get_google_fonts()
    {
...
add_action('wp_enqueue_scripts', 'minimal_get_google_fonts');
...
function minimal_theme_options(){
    ...
    /* Custom CSS */
	//wp_enqueue_style('minimal-custom-style', get_template_directory_uri() . '/css/custom.css');
    wp_add_inline_style('minimal-custom-style', $custom_css);
    
}
add_action('wp_enqueue_scripts', 'minimal_theme_options');
        ...
        if ( class_exists( 'woocommerce' ) ) {
            ...
            $items .= '<li class="' . esc_attr( $css_class ) . '">';
                $items .= '<a class="cart-contents" href="' . esc_url( WC()->cart->get_cart_url() ) . '"><i class="fa fa-shopping-bag" aria-hidden="true"></i> ';
                    $items .= wp_kses_data( WC()->cart->get_cart_total() ) . ' - <span class="count">' .  wp_kses_data( sprintf( _n( '%d item', '%d items', WC()->cart->get_cart_contents_count(), 'minimal' ), WC()->cart->get_cart_contents_count() ) ) . '</span>';
                $items .= '</a>';
            $items .= '</li>';
        }
        // Easy Digital Downloads
            ...
            $items .= '<li class="' . esc_attr( $css_class ) . '">';
                $items .= '<a class="cart-contents" href="' . esc_url( edd_get_checkout_uri() ) . '"><i class="fa fa-shopping-bag" aria-hidden="true"></i> ';
                    $items .= wp_kses_data( edd_cart_subtotal() ) . ' - <span class="count">' .  wp_kses_data( sprintf( _n( '%d item', '%d items', edd_get_cart_quantity(), 'minimal' ), edd_get_cart_quantity() ) ) . '</span>';
                $items .= '</a>';
            $items .= '</li>';
        }
    }
...
function minimal_custom_excerpt_more($more) {
      global $post;
      if ($post->post_type == 'post'){
   ...
   add_filter('excerpt_more', 'minimal_custom_excerpt_more');
/*animated number counter script*/



/* +========================ot_theme_mode  Added By Dev8 on 15/09/2017===========================================*/
function minimal_theme_file() {

	require_once( get_template_directory() . '/inc/minimal-theme-option.php' );

}
add_action('wp_head', 'minimal_theme_file');
/**
...
function ocdi_import_files() {
  return array(
    array(
      'import_file_name'           => 'Minimal Studio',
      'categories'                 => array('Creative'),
      'import_file_url'            => 'http://html-templates.multipurposethemes.com/wordpress/themes/data/minimalstudio/01/demo-content.xml',
      'import_widget_file_url'     => 'http://html-templates.multipurposethemes.com/wordpress/themes/data/minimalstudio/01/widgets.wie',
      'import_customizer_file_url' => 'http://html-templates.multipurposethemes.com/wordpress/themes/data/minimalstudio/01/customizer.dat',
      'import_preview_image_url'   => 'http://html-templates.multipurposethemes.com/wordpress/themes/data/minimalstudio/01/screenshot.png',
      'import_notice'              => __( 'After you import this demo, you will have to setup the slider separately.', 'maximum' ),
      'preview_url'                => 'https://www.multipurposethemes.com/wordpressdemo/minimalstudio/',
    ),
  );
}
