...
function custom_meta_box_markup()
{
   $values = get_post_custom( $post->ID );
    $selected = isset( $values['mpt_overlay_header'] ) ? esc_attr( $values['mpt_overlay_header'][0] ) : 'Default';
     ...
     $slide = isset( $values['mpt_include_slide'] ) ? esc_attr( $values['mpt_include_slide'][0] ) : 'Default';
...
?>
                ...
                <p class="post-attributes-label-wrapper"><label class="post-attributes-label" for="mpt_include_slide">Include in One Page Slide</label></p>
                <select name="mpt_include_slide" id="mpt_include_slide">
                    <option value="Default" <?php selected( $slide, 'Default' ); ?>>Default</option>
                    <option value="Yes" <?php selected( $slide, 'Yes' ); ?>>Yes</option>
                    <option value="No" <?php selected( $slide, 'No' ); ?>>No</option>
                </select>
<?php 
}
...
function mpt_meta_box_save( $post_id )
{
    ...
    if( isset( $_POST['mpt_include_slide'] ) )
        update_post_meta( $post_id, 'mpt_include_slide', esc_attr( $_POST['mpt_include_slide'] ) );
